...
**Phase 6: Frontend** — in progress (6a–6c complete, search complete, backlog complete, banner/navigation complete, blockers complete, snooze detail page complete; deploy in progress — API-side prod-readiness complete (`/api` prefix unification, `/healthz`, `trust proxy`, `express.static` SPA serving); Dockerfile frontend build stage complete (`build-web` stage emits `packages/web/dist/`, copied into final image; `ENV WEB_DIST_DIR=/app/packages/web/dist` set explicitly, `ENV NODE_ENV=production` set in final stage); env var hygiene complete (secrets audit clean, required prod vars documented in `docs/DEPLOYMENT.md`); full-stack `podman compose up --build` verified end-to-end (healthz, SPA, login + task CRUD); remaining: live deploy to host + Atlas IP allowlist for host's egress; one feature still independent: snooze task list section).
