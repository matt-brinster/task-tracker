...
**Phase 6: Frontend** — in progress (6a–6c complete, search complete, backlog complete, banner/navigation complete, blockers complete, snooze detail page complete; deploy complete — API-side prod-readiness done (`/api` prefix unification, `/healthz`, `trust proxy`, `express.static` SPA serving); Dockerfile frontend build stage done (`build-web` stage emits `packages/web/dist/`, copied into final image; `ENV WEB_DIST_DIR=/app/packages/web/dist` set explicitly, `ENV NODE_ENV=production` set in final stage); env var hygiene done (secrets audit clean, required prod vars documented in `docs/DEPLOYMENT.md`); full-stack `podman compose up --build` verified end-to-end (healthz, SPA, login + task CRUD); live on Render free Web Service with Atlas M0 (2026-05-20), Atlas allowlist contains Render's two shared free-tier egress IPs. Known gap surfaced by deploy: **first-admin bootstrap requires shell access**, which Render free does not provide — the `docker compose exec ... provision-cli.js` pattern documented in `docs/DEPLOYMENT.md` doesn't work in prod; first admin was seeded by writing an invitation document directly into Atlas as a stopgap. Remaining feature: snooze task list section. Remaining infra: proper no-shell bootstrap mechanism (see Bootstrap admin in the project plan).
