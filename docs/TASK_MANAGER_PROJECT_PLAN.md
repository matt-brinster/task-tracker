...
- **Remaining features** — these are independent of each other and can be tackled in any order:
  ...
  - **Admin** — Admin-only UI and API. `isAdmin` on User model ✅, admin auth middleware (403 for non-admins) ✅. API: `POST /admin/users` ✅ (creates user, returns `invitationKey`); `GET /users/me` ✅ (returns `isAdmin` for frontend gating); `GET /admin/users` (list users) — not built; `POST /admin/users/:id/send-code` (sends email code) — gated on email system. Frontend: create-user form is embedded as an `AdminSection` inside the Settings page (no dedicated admin page), only rendered when `currentUser.isAdmin` is true ✅. Initially displays the invitation key for the admin to hand off; once email codes land, the key display goes away and the form triggers a code send instead (see the Email codes item). Independent of task features; depends on deploy only if email is needed.
  - **Email codes** — Replace invitation keys with 8-digit numeric codes. New `email_codes` collection (`id`, `userId`, `codeHash`, `createdAt`, `expiresAt`, `redeemedAt`). New domain type and factory (`createEmailCode`). Repository: `insertEmailCode`, `findEmailCodeByHash`, `markRedeemed`. New endpoint `POST /auth/redeem-code` (accepts `{ code }`, validates, creates session). Email service: Resend or SendGrid (both have free tiers sufficient for this scale). Local dev: console log transport (log code to stdout) or Mailpit in docker-compose — no real emails in dev/test. Admin triggers send from admin page. Codes: 8-digit numeric, single-use, 4-hour expiry. Admin response cleanup: revisit `POST /admin/users` — once codes are emailed directly to the user, the response no longer needs to return `invitationKey`. Update `ProvisionUserResponse` in `packages/web/src/types.ts` and remove the invite-key display, copy button, and related state from `AdminSection`. **Depends on:** admin page (for send UI), deploy (for email service config).
