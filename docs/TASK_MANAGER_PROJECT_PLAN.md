...
- **Remaining features** — these are independent of each other and can be tackled in any order:
  - **Backlog** — Queue toggle on task detail page (todo/backlog). Backlog section or separate view on task list. API: `POST /tasks/:id/queue` (already exists).
  - **Blockers** — Blocker display on task detail page (list of blocking tasks with completion status). Add blocker (task picker/search to select from open tasks). Remove blocker. Blocked section on task list (tasks with unresolved blockers). API: `POST /tasks/:id/blockers`, `POST /tasks/:id/blockers/remove` (already exist). Note: the list page already filters out blocked tasks — this feature makes them visible in their own section. We will need blocker title fan-out when a title update endpoint is added.
  - **Snooze** — Snooze button with date picker on task detail page. Wake button (clear snooze). Snoozed section on task list (tasks where `snoozedUntil > now`). API: `POST /tasks/:id/snooze`, `POST /tasks/:id/wake` (already exist). Note: the list page already filters out snoozed tasks — this feature makes them visible in their own section.
  - **Search** — Search input on task list (or separate view). Calls `GET /tasks/open/search?q=...`, debounced. Results displayed in same task row format. API already exists.
  - **Prioritization** — Manual reordering of tasks in the list. Likely a `position` or `sortOrder` field on Task. Drag-and-drop or move up/down buttons on the list page. Needs a new API endpoint to update sort order (e.g. `PATCH /tasks/:id` with `{ position }`, or a bulk reorder endpoint). Consider: ordering scoped per queue, fractional indexing (to avoid renumbering all tasks on every move).
