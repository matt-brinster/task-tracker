...
- **Remaining features** — completed items above were independent; the items below have an intentional order (Atlas migration → deploy → email-based login) so a migration bug and a deploy bug can't disguise each other, and email deliverability is built against real prod constraints (Atlas Search was originally sequenced here before deploy but has been deferred to Future Possibilities):
  ...
  - **Deploy** ✅ — Target host: **Render** free Web Service (Atlas free M0 for the DB). Originally scoped for Koyeb (free tier, always-on, no cold starts) but Koyeb dropped their always-free tier mid-deploy and their cheapest paid plan is ~$30/mo — too much for a family app. Render free has cold starts (~15 min idle → ~30s warmup on next request); acceptable tradeoff for the **Live-with-it period** since this is a few-times-a-day usage pattern. The Dockerfile + DEPLOYMENT.md are intentionally platform-generic, so the swap was almost entirely off-repo. API-side prod-readiness ✅: `/api` prefix unification (mounts under `/api` in Express, Vite proxy no longer rewrites — dev and prod paths identical, eliminating the "works in dev, breaks in prod" class of bug); `/healthz` liveness endpoint (registered before the request logger); `trust proxy` set with `TRUST_PROXY_HOPS` env override (default 1, safe values 0/1/2; **never** `true` — `X-Forwarded-For` is client-settable and `true` lets an attacker spoof IPs past the per-IP auth limiter); `express.static(webDistDir)` serves the built SPA from Express in the single prod container (no nginx — simpler; revisit if static asset perf becomes an issue), minimal (no history fallback — app has no client routing); `webDistDir` resolves via `WEB_DIST_DIR` env or module-relative default. Dockerfile frontend build stage ✅: split `build-api` / `build-web` stages (each runs `npm ci` only for its own workspace, so a misplaced dependency surfaces instead of being hidden by workspace hoisting — caught `@dnd-kit/react` declared in api, moved to web); final image copies `packages/web/dist/` and sets `ENV WEB_DIST_DIR=/app/packages/web/dist` explicitly so the runtime contract is visible in the Dockerfile rather than implicit in a module-relative path. Full-stack `podman compose up --build` verified end-to-end (healthz, SPA, login + task CRUD); first-admin provisioning runs inside the container (`docker compose exec app node packages/api/dist/admin/provision-cli.js --email ...`) — pattern translates directly to any host's "exec into a running instance" command (Render Shell, `fly ssh console`, etc.). Env var hygiene ✅: secrets audit clean (no `.env` ever committed — only `*.example` files containing the `devpassword` local-dev placeholder); `NODE_ENV=production` set in the Dockerfile final stage; required prod vars (`MONGO_URI` with mandatory DB-name-in-path, optional `PORT`/`TRUST_PROXY_HOPS`) and Dockerfile-baked vars (`WEB_DIST_DIR`) documented in [DEPLOYMENT.md](DEPLOYMENT.md), along with Atlas allowlist guidance (static IPs / CIDR / `0.0.0.0/0` fallback) and in-container first-admin provisioning. Live as of 2026-05-20: Render Web Service builds from `main` via the Dockerfile; `MONGO_URI` set as a secret env var; Atlas allowlist contains Render's two shared free-tier egress IPs (strictly better than `0.0.0.0/0` even though shared with other Render free tenants — fragility caveat: free-tier egress IPs can rotate without notice, so a future mystery connection failure should check the allowlist first). Surprise discovered during deploy: **Render free has no shell/SSH access**, so the `docker compose exec ... provision-cli.js` first-admin pattern documented in [DEPLOYMENT.md](DEPLOYMENT.md) doesn't work in prod. Worked around for now by writing an invitation document directly into Atlas via the data browser to seed the first admin; proper fix tracked in the next bullet. Deploy with invitation-key auth still in place — friends get keys by hand until email login lands. **Live-with-it period:** use it in prod for ~1 week before starting email-codes work; bugs surfaced in real use are usually different from bugs surfaced in dev.
  - **Bootstrap admin (no-shell host)** — Render free has no SSH/shell, so the `docker compose exec ... provision-cli.js` first-admin pattern documented in [DEPLOYMENT.md](DEPLOYMENT.md) doesn't work in prod. Current workaround (writing an invitation document directly into Atlas) seeds the first user but skips the domain factory, bypasses index assumptions, and is awkward to repeat after a wipe/redeploy. Three candidate solutions, in order of friction:
    - **Run provision-CLI locally against the prod `MONGO_URI`.** Zero new code. Point a temporary local `.env` at the Atlas connection string and run the existing `provision-cli.ts`. Requires adding the dev IP to the Atlas allowlist (alongside Render's egress IPs) for the duration, then removing it. Friction: low per use, but no automation — same steps every redeploy.
    - **Env-var bootstrap on startup.** New `BOOTSTRAP_ADMIN_EMAIL` env var; `index.ts` checks at startup whether any users exist, and if not, provisions an admin with that email and prints the invitation key to logs (Render's log viewer works without shell). Self-disabling — only fires on an empty DB, so leaving the var set is safe. Smallest code addition; no new endpoint, no new attack surface, no new secret handling. Compose-friendly for local dev too.
    - **One-shot bootstrap endpoint.** `POST /admin/bootstrap` guarded by a shared secret, refuses if any users already exist. Most code, most attack surface, and the secret is yet another thing to manage — probably overkill for a once-per-deploy operation.
    Leaning toward the env-var bootstrap: contained, idempotent, matches a common "first-run setup" pattern, and works on any host with env vars + log access. Decision deferrable past the **Live-with-it period** — the direct DB-write is sufficient for current use, and **Email codes** may slightly reshape the bootstrap surface (the first admin still needs a session-creating credential, but the *invitation key* form may change).
