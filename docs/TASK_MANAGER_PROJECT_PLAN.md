...
- **Remaining features** — completed items above were independent; the items below have an intentional order (Atlas migration → Atlas Search → deploy → email-based login) so a migration bug, a search bug, and a deploy bug can't disguise each other, and email deliverability is built against real prod constraints:
  ...
  - **Migrate to Atlas** — Swap local MongoDB for Atlas (M0 free tier) in dev first, then prod. Scope is strictly the migration — no new features layered in. Tasks: create M0 cluster, update `MONGO_URI` handling for the SRV connection string, configure IP allowlist (dev: current IP; prod: deploy provider's egress IPs once known), verify `ensureIndexes()` still builds all indexes against Atlas, run full test suite against Atlas, confirm `podman compose up` still works locally (test env can stay on local Mongo to keep CI/test fast). Risk-isolated so an Atlas bug and a search bug can't disguise each other.
  - **Atlas Search (fuzzy search)** — Now that we're on Atlas, swap `$text` for `$search` in `searchOpenTasks` and `searchAllTasks` (`task_repository.ts`) and remove the `tasks_text` index from `indexes.ts` (Atlas Search indexes are defined in the Atlas UI, not via the Mongo driver). Behavior change: prefix/fuzzy/typo-tolerant matching instead of whole-word only — users find results mid-type. See the TODO comment in `task_repository.ts` for the query shape. Note: Atlas Search is a new pattern (separate index type, defined out-of-band) — worth pausing to understand before implementing.
  - **Deploy** — Target host: **Koyeb** (free tier, always-on, no cold starts; Atlas free M0 for the DB). Build static frontend output, update Dockerfile / docker-compose to serve frontend (likely from Express — simpler than a second nginx container; revisit if static asset perf becomes an issue). Verify `docker compose up --build` runs full stack locally. Prod-readiness work: env var hygiene (no committed secrets, document required vars), `/healthz` endpoint for Koyeb's health checks, structured request logging review, confirm rate limiter behavior behind a proxy (`trust proxy` setting), Atlas IP allowlist for Koyeb egress. Deploy with invitation-key auth still in place — friends get keys by hand until email login lands. **Live-with-it period:** use it in prod for ~1 week before starting email-codes work; bugs surfaced in real use are usually different from bugs surfaced in dev.
