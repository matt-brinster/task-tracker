...
describe('completeTask', () => {
  ...
  it('preserves all other fields', () => {
    const task = createTask('Buy milk', 'from the shop', 'backlog', new Set(['id-1']))
    const now = new Date('2026-03-10T12:00:00Z')
    const result = completeTask(task, now)
...
describe('reopenTask', () => {
  it('clears completedAt', () => {
    const task = completeTask(createTask('Buy milk'), new Date())
    const result = reopenTask(task)
    expect(result.completedAt).toBeNull()
  })

  it('does not mutate the original task', () => {
    const completed = completeTask(createTask('Buy milk'), new Date())
...
describe('wakeTask', () => {
  it('clears snoozedUntil', () => {
    const task = snoozeTask(createTask('Buy milk'), new Date('2026-03-15T09:00:00Z'))
    const result = wakeTask(task)
    expect(result.snoozedUntil).toBeNull()
  })

  it('does not mutate the original task', () => {
    const snoozed = snoozeTask(createTask('Buy milk'), new Date('2026-03-15T09:00:00Z'))
    wakeTask(snoozed)
...
describe('addBlockerIds', () => {
  it('adds blocker IDs to an empty set', () => {
    const task = createTask('Buy bread')
  ...
  it('merges with existing blocker IDs', () => {
    const task = createTask('Buy bread', '', 'todo', new Set(['existing']))
  ...
  it('deduplicates IDs already present', () => {
    const task = createTask('Buy bread', '', 'todo', new Set(['abc123']))
    const result = addBlockerIds(task, new Set(['abc123']))
    expect(result.blockerIds).toEqual(new Set(['abc123']))
  })
  ...
  it('preserves all other fields', () => {
    const task = createTask('Buy bread', 'from the bakery', 'backlog')
    const result = addBlockerIds(task, new Set(['abc123']))
    expect(result.id).toBe(task.id)
    expect(result.title).toBe(task.title)
    expect(result.details).toBe(task.details)
...
describe('removeBlockerIds', () => {
  ...
  it('removes only the specified IDs, leaving others', () => {
    const task = createTask('Buy bread', '', 'todo', new Set(['a', 'b', 'c']))
  ...
  it('preserves all other fields', () => {
    const task = createTask('Buy bread', 'from the bakery', 'backlog', new Set(['abc123']))
    const result = removeBlockerIds(task, new Set(['abc123']))
    expect(result.id).toBe(task.id)
    expect(result.title).toBe(task.title)
    expect(result.details).toBe(task.details)
